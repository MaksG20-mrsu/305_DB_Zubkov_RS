...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT DISTINCT m.title, u1.name as name1, u2.name as name2 FROM ratings r CROSS JOIN ratings r2 Join users u1 on r.user_id = u1.id JOIN users u2 on r2.user_id = u2.id join movies m on r.movie_id = m.id WHERE r.user_id < r2.user_id LIMIT 100;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT DISTINCT m.title, u.name, r.rating, date(r.timestamp, 'unixepoch') AS review_date FROM ratings r JOIN users u ON r.user_id = u.id JOIN movies m ON r.movie_id = m.id GROUP BY u.name ORDER BY r.timestamp ASC LIMIT 10;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "WITH avg_ratings AS (SELECT m.id, m.title, m.year, AVG(r.rating) AS avg_rating FROM movies m JOIN ratings r ON m.id = r.movie_id GROUP BY m.id, m.title, m.year), min_max AS (SELECT MIN(avg_rating) AS min_rating, MAX(avg_rating) AS max_rating FROM avg_ratings) SELECT ar.year, ar.title, ar.avg_rating, CASE WHEN ar.avg_rating = mm.max_rating THEN 'Да' ELSE 'Нет' END AS Рекомендуем FROM avg_ratings ar CROSS JOIN min_max mm WHERE ar.avg_rating = mm.min_rating OR ar.avg_rating = mm.max_rating AND ar.year is not NULL ORDER BY ar.year, ar.title;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT m.year, m.title, ROUND(AVG(r.rating), 2) AS средняя_оценка, COUNT(DISTINCT r.user_id) AS количество_пользователей FROM movies m JOIN ratings r ON m.id = r.movie_id WHERE m.year is not null GROUP BY m.year, m.title ORDER BY m.year, m.title LIMIT 20;"
echo " "
